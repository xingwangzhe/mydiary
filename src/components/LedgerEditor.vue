<template>
  <div class="min-h-screen bg-surface p-8">
    ...
    <header class="glass-nav fixed top-0 left-0 right-0 z-50 px-6 py-4 flex items-center justify-between">
      ...
      <div class="flex items-center space-x-4">
        <button @click="saveLedger" class="bg-primary-gradient text-on-primary px-6 py-2 rounded-full font-medium hover:bg-primary/90 transition-colors">
          保存
        </button>
      </div>
    </header>
    ...
    <main class="pt-24 pb-12 max-w-4xl mx-auto">
      ...
      <div class="space-y-6 mb-8">
        <!-- 金额 -->
        <div>
          <label class="block text-sm font-medium text-on-surface/80 mb-2 font-['Inter']">金额</label>
          <input 
        ...
        <!-- 类型 -->
        <div>
          <label class="block text-sm font-medium text-on-surface/80 mb-2 font-['Inter']">类型</label>
        ...
        <!-- 日期 -->
        <div>
          <label class="block text-sm font-medium text-on-surface/80 mb-2 font-['Inter']">日期</label>
          <input 
            v-model="ledger.date" 
            type="date" 
            class="w-full text-body-md text-on-surface bg-surface-container-highest p-4 rounded-lg focus:bg-white focus:shadow-sm focus:outline-none transition-all font-['Inter']"
