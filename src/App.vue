<template>
  <div class="min-h-screen bg-surface" :class="{ dark: isDarkMode }">
    ...
    <div v-else>
      <!-- 侧边导航栏 -->
      <aside
        class="h-full w-72 fixed left-0 top-0 bg-[#f5f3ef] dark:bg-stone-900 border-none flex flex-col p-6 space-y-8 z-50 overflow-y-auto"
      >
        <div class="mb-8">
          <h1 class="text-2xl font-bold text-[#4a654e] dark:text-[#8ba88e] font-['Manrope']">
            日记
          </h1>
        ...
        <div class="pt-6 border-t border-outline-variant/10">
          <button
            class="w-full bg-primary-gradient text-on-primary rounded-full py-4 px-6 flex items-center justify-center gap-2 font-['Inter'] font-semibold active:scale-95 transition-transform"
      ...
      <main class="ml-72 min-h-screen">
        <!-- 顶部导航栏 -->
        <header
          class="sticky top-0 z-40 w-full bg-[#fbf9f5]/80 backdrop-blur-xl flex justify-between items-center px-12 py-8"
        >
          <div
            class="flex items-center gap-4 bg-surface-container-highest/50 px-6 py-3 rounded-full w-96 group focus-within:bg-white transition-all duration-300"
          >
            <span class="material-symbols-outlined text-stone-400 group-focus-within:text-primary"
              >search</span
            >
            <input
          ...
          <div class="flex items-center gap-8">
            <div class="flex gap-6">
              <span
                class="material-symbols-outlined text-stone-400 cursor-pointer hover:opacity-70 transition-opacity active:opacity-50"
            ...
            <div class="flex flex-col items-end">
              <span class="text-xs font-['Inter'] uppercase tracking-widest text-[#4a654e]">{{
                activeNav === 'tasks'
                  ? '任务管理'
                  : activeNav === 'ledger'
                    ? '财务概览'
        ...
        <div class="px-12 py-12 max-w-7xl mx-auto space-y-16">
          ...
          <section class="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div class="space-y-4">
              <h2
                class="text-[56px] leading-tight font-['Manrope'] font-light tracking-tight text-on-surface"
              >
                {{
                  activeNav === 'tasks'
                    ? '任务管理'
              ...
              <p class="text-lg font-['Inter'] text-secondary font-light max-w-md opacity-80">
                {{
                  activeNav === 'tasks'
                    ? '合理规划任务，提高工作效率，实现目标。'
                    : activeNav === 'ledger'
                      ? '记录每一笔收支，掌握财务状况，实现财务自由。'
                      : '在这里，记录每一刻内心的宁静与思考。让生活在笔尖缓缓流淌，成就更好的自己。'
                }}
              </p>
            </div>
            <button
          ...
          <div class="grid grid-cols-12 gap-8 items-start">
            ...
            <section class="col-span-12 lg:col-span-8 space-y-12">
                  ...
                  >
                    <div class="flex items-start gap-4">
                      <label class="relative inline-flex items-center cursor-pointer pt-1">
                        ...
                        />
                        <div
                          class="w-6 h-6 bg-surface-container-highest peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"
                        ></div>
                      </label>
                      <div class="flex-1 space-y-2">
                        <h4
                          class="text-lg font-['Manrope'] font-semibold text-on-surface"
                          :class="{ 'line-through text-stone-400': task.completed }"
                        >
                          {{ task.title }}
                        </h4>
                        <p
                          class="text-sm text-on-surface-variant font-['Inter']"
                          :class="{ 'line-through text-stone-400': task.completed }"
                        >
                          {{ task.description }}
                        </p>
                        <div class="flex items-center gap-4 mt-2">
                          ...
                          <span class="text-xs text-stone-400 font-['Inter']"
                            >截止：{{ formatDate(task.dueDate) }}</span
                          >
                        </div>
                      </div>
                      <div class="flex space-x-2">
                        ...
                        >
                          <span
                            class="material-symbols-outlined text-stone-400 hover:text-primary transition-colors"
                            >edit</span
                          >
                        </button>
                        <button
                        ...
                        >
                          <span
                            class="material-symbols-outlined text-stone-400 hover:text-error transition-colors"
                            >delete</span
                          >
                        </button>
                      </div>
              ...
              <div v-if="activeNav === 'ledger'" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div class="bg-surface-container-low rounded-2xl p-6 editorial-shadow">
                    ...
                    <p class="text-2xl font-['Manrope'] font-bold text-green-600">
                      ¥{{ financialSummary.totalIncome.toFixed(2) }}
                    </p>
                  </div>
                  <div class="bg-surface-container-low rounded-2xl p-6 editorial-shadow">
                    <h4 class="text-sm font-['Inter'] text-on-surface-variant mb-2">总支出</h4>
                    <p class="text-2xl font-['Manrope'] font-bold text-red-600">
                      ¥{{ financialSummary.totalExpense.toFixed(2) }}
                    </p>
                  </div>
                  <div class="bg-surface-container-low rounded-2xl p-6 editorial-shadow">
                    ...
                    <p
                      :class="[
                        'text-2xl font-[Manrope] font-bold',
                ...
                <div class="space-y-4">
                  ...
                  >
                    <div class="flex items-start justify-between">
                      <div class="flex-1 space-y-2">
                        <h4 class="text-lg font-['Manrope'] font-semibold text-on-surface">
                          {{ ledger.title }}
                        </h4>
                        <div class="flex items-center gap-4">
                          <span
                            class="text-xs font-['Inter'] font-semibold bg-primary/10 text-primary px-3 py-1 rounded-full"
                            >{{ ledger.category }}</span
                          >
                          <span class="text-xs text-stone-400 font-['Inter']">{{
                            formatDate(ledger.date)
                          }}</span>
                        </div>
                        <p class="text-sm text-on-surface-variant font-['Inter']">
                          {{ ledger.description }}
                        </p>
                      </div>
                      <div class="flex flex-col items-end justify-between">
                        <p
                          :class="[
                            'text-lg font-[Manrope] font-bold',
                            ledger.type === 'income' ? 'text-green-600' : 'text-red-600',
                          ]"
                        >
              ...
              <div v-else-if="activeNav === 'journal'" class="space-y-8">
                <div class="flex items-center justify-between">
                  ...
                  <span
                    class="text-xs font-['Inter'] uppercase tracking-widest text-stone-400 border-b border-stone-200 pb-1 cursor-pointer hover:text-primary"
                    @click="handleViewAllJournals"
                    >查看全部日志</span
                  >
                </div>
                <div class="space-y-8">
                  ...
                  >
                    <div
                      class="w-full md:w-48 h-32 flex-shrink-0 rounded-lg overflow-hidden bg-surface-container"
                    >
                      <img
                    ...
                    <div class="flex-1 space-y-3">
                      <div class="flex items-center gap-3">
                        <span
                          class="text-xs font-['Inter'] font-semibold bg-primary-fixed text-on-primary-fixed px-3 py-1 rounded-full"
                          >{{ journal.mood }}</span
                        >
                        <time class="text-sm text-stone-400 font-['Inter']">{{
                          formatDate(journal.date)
                        }}</time>
                      </div>
                      <h4 class="text-xl font-['Manrope'] font-semibold text-on-surface">
                        {{ journal.title }}
                      </h4>
                      <p
                        class="text-on-surface-variant font-['Inter'] line-clamp-2 leading-relaxed"
                      >
                        {{ journal.content }}
                      </p>
                    </div>
                    <div class="flex flex-col items-end justify-between">
                      <span
                        class="material-symbols-outlined text-primary-container group-hover:text-primary transition-colors"
            ...
            <aside class="col-span-12 lg:col-span-4 space-y-8">
              ...
              <div
                v-if="activeNav === 'tasks'"
                class="bg-surface-container-low rounded-2xl p-8 space-y-6"
              >
                <div class="flex items-center justify-between">
                ...
                <div class="space-y-4">
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-lg font-['Manrope'] font-semibold">{{
                      completedTasksCount
                    }}</span>
                  </div>
                  <div class="flex items-center justify-between">
              ...
              >
                ...
                <div class="space-y-4">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-lg font-['Manrope'] font-semibold text-green-600"
                      >¥{{ financialSummary.totalIncome.toFixed(2) }}</span
                    >
                  </div>
                  <div class="flex items-center justify-between">
                    <span class="text-sm font-['Inter'] text-on-surface-variant">总支出</span>
                    <span class="text-lg font-['Manrope'] font-semibold text-red-600"
                      >¥{{ financialSummary.totalExpense.toFixed(2) }}</span
                    >
                  </div>
                  <div class="flex items-center justify-between">
                  ...
                  <div class="w-full bg-surface-container-highest rounded-full h-2 mt-4">
                    <div
                      class="bg-primary-gradient h-2 rounded-full transition-all duration-500"
                      :style="{
                        width: `${financialSummary.totalIncome > 0 ? (financialSummary.balance / financialSummary.totalIncome) * 100 : 0}%`,
                      }"
                    ></div>
                  </div>
                  <p class="text-sm font-['Inter'] text-on-surface-variant text-center">
                    结余率：{{
                      financialSummary.totalIncome > 0
                        ? ((financialSummary.balance / financialSummary.totalIncome) * 100).toFixed(
                            1,
                          )
                        : 0
                    }}%
              ...
              <!-- 今日心情图表 -->
              <div
                v-else-if="activeNav === 'journal' || activeNav === 'home'"
                class="bg-surface-container-low rounded-2xl p-8 space-y-6"
              >
                <div class="flex items-center justify-between">
                ...
                <div class="flex items-end justify-between h-32 gap-3 px-2">
                  <div class="flex-1 bg-primary/20 rounded-t-lg relative group h-[40%]">
                    <div
                      class="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-on-surface text-white text-[10px] py-1 px-2 rounded"
                    >
                      40%
                    </div>
                  </div>
                ...
                <p
                  class="text-sm font-['Inter'] text-on-surface-variant italic border-l-2 border-primary-container pl-4 py-1"
                >
                  "波动起伏是生命的常态，正如潮汐。"
                </p>
              </div>
              ...
              <div class="bg-secondary-container/30 rounded-2xl p-8 space-y-6">
                <h3 class="text-lg font-['Manrope'] font-semibold">
                  {{ activeNav === 'tasks' ? '任务趋势' : '记录习惯' }}
                </h3>
                ...
                <div v-else class="grid grid-cols-4 gap-4">
                  <div
                    class="aspect-square rounded-full border-2 border-primary/20 flex items-center justify-center text-primary font-bold text-xs"
                  >
                    一
                  </div>
                  <div
                  ...
                  <div
                    class="aspect-square rounded-full bg-primary-gradient text-white flex items-center justify-center font-bold text-xs"
                  >
                    三
                  </div>
                  <div
                ...
                <div class="flex items-center gap-3 pt-4">
                  <div class="flex -space-x-2">
                    <img
                      ...
                      src="https://lh3.googleusercontent.com/aida-public/AB6AXuCdBpiT_NjbBfLY0sajrDMo8PyElM8qfOyHQDr9KVIpo2RNWfe5ruhNfUEfiAE9-hsDT2-svkXD1c6UodpXHbznuXDDqyAAT7jK6Fo0kvsFWLsQbMfhGDD5VivOWTD5tmPqiCt8is14R6ab1Rvz87xiajD4qZ57GEQXOzW_PviMumzX2qX_ZDBV1QFh3tvmLxUTIefD0774saMAlUwlW65oMjFOrJUKplp_vMbbc2XrdGq-GzU_rn1xUX9AXc1X8TRqVDV16MXxB7w"
                      alt="用户头像"
                    />
                    <img
                      class="w-6 h-6 rounded-full border-2 border-background"
                      src="https://lh3.googleusercontent.com/aida-public/AB6AXuD3IFk9z9vnUaFpfGyi3m7PjUkwf8y7QCZWcQGXISuogPNCAP_TXdwPpJHGzDYsXwcKiH64Gm2cahyNKsjL9TZZfglmvEiD3t44pN1Uq9JJoB_ninRHomH2QjFderrQpW2w5-J9oc5hgvkIdETVfZMXGIufgqCT1jfGCaty10CYNzpNwO7TXrkfhw1ExHU3XXBt3YKnAAvjmewyBfZh7QnCh3oKpOqgDWbNVACTONAHiDqGBG8tHKh-sb5wx49m-0waCQVO8S6W_PM"
                      alt="用户头像"
                    />
                  </div>
              ...
              <!-- 引用卡片 -->
              <div
                class="relative rounded-2xl overflow-hidden h-64 flex items-center justify-center p-8 text-center group"
              >
                <img
